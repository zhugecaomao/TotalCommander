...
void StringSplit(wstring &Result, wstring &SplitText, const wstring &SplitChar, int SplitLastMatch) {
    int Pos;
    if(SplitLastMatch) Pos = int(SplitText.find_last_of(SplitChar));
    else               Pos = int(SplitText.find(SplitChar));
    if(Pos == -1) {
        Result = SplitText.substr(0, SplitText.length());
        //wstring *temp = new wstring(SplitText.substr(0, SplitText.length())); // Memory Leak
        SplitText = L"";
        return;
    }
    Result = SplitText.substr(0, Pos);
    //wstring *temp = new wstring(SplitText.substr(0, Pos)); // Memory Leak
...
int wdx_class::LoadEntryFromIni(int Entry, int &LibraryCount, int &FieldCount) {
    ...
    // try to load library
#ifdef WIN64
    if(GetPrivateProfileString(L"wdx64",KeyIndex.c_str(),L"",wcBuffer,BSize,wsIniFile.c_str())==0) return -1;
#else
    ...
    wstring wsFieldnames = L"";
    StringSplit(wsFieldnames, wdx_plugin[LibraryCount - 1].Name, L"@");
    wdx_plugin[LibraryCount - 1].Lib = LoadLibrary(wdx_plugin[LibraryCount - 1].Name.c_str());
    if(!wdx_plugin[LibraryCount - 1].Lib) return 0;
    wstring Dummy = L"";
    StringSplit(Dummy, wdx_plugin[LibraryCount - 1].Name, L"\\", 1);
    ...
    if(wdx_plugin[LibraryCount - 1].F_ContentSetDefaultParams) {
        ...
        try { wdx_plugin[LibraryCount - 1].F_ContentSetDefaultParams(&Version); } catch(...) { if(DebugLevel(1)) DebugMessage(L"Error in Plugin: \"" + wdx_plugin[LibraryCount - 1].Name + L"\" calling \"ContentSetDefaultParams\""); }
    }
    ...
    if(wdx_plugin[LibraryCount - 1].F_ContentGetSupportedField) for(int i = 0;; i++) {
        ...
        try { Result = wdx_plugin[LibraryCount - 1].F_ContentGetSupportedField(i, cBuffer, cBuffer2, BSize); } catch(...) { if(DebugLevel(1)) DebugMessage(L"Error in Plugin: \"" + wdx_plugin[LibraryCount - 1].Name + L"\" calling \"ContentGetSupportedField\""); continue; }
...
void wdx_class::LoadFromIni() {
...
#ifdef WIN64
    if(GetPrivateProfileString(L"wdx64",L"wdx_groups",L"",wcBuffer,BSize,wsIniFile.c_str())!=0){
#else
    if(GetPrivateProfileString(L"wdx", L"wdx_groups", L"", wcBuffer, BSize, wsIniFile.c_str()) != 0) {
        ...
        while(IniText.length()) {
            Groups.resize(++i);
            wstring IniTextGroup = L"";
            StringSplit(IniTextGroup, IniText, L"|");
            while(IniTextGroup.length()) {
                wstring IniTextNumber = L"";
                StringSplit(IniTextNumber, IniTextGroup, L",");
...
void wdx_class::Create_wdxStrings(const wstring &wsFilename, int INI_case_sensitive) {
    ...
    wstring wsCurrentPath = L"";
    StringSplit(wsCurrentPath, wstring(wsFilename), L"\\", 1);
    if(wsCurrentPath != wsLastPath) {
        ...
        for(int i = 0; i < (int)wdx_plugin.size(); i++) if(wdx_plugin[i].Lib) {
            if(wdx_plugin[i].F_ContentSendStateInformationW) {
                ...
                try { wdx_plugin[i].F_ContentSendStateInformationW(1, wcBuffer); } catch(...) { if(DebugLevel(1)) DebugMessage(L"Error in Plugin: \"" + wdx_plugin[i].Name + L"\" calling \"ContentSendStateInformationW\""); }
                ...
                try { wdx_plugin[i].F_ContentSendStateInformation(1, cBuffer); } catch(...) { if(DebugLevel(1)) DebugMessage(L"Error in Plugin: \"" + wdx_plugin[i].Name + L"\" calling \"ContentSendStateInformation\""); }
            }
        }
    }
...
int wdx_class::ContentGetValue(int n, int Unitindex) {
    ...
    if(wdx_plugin[wdx_field[n].PluginIndex].F_ContentGetValueW) {                 // plugin with Unicode filename support
        ...
        try { Result = wdx_plugin[wdx_field[n].PluginIndex].F_ContentGetValueW(wcBuffer, wdx_field[n].FieldIndex, Unitindex, cBuffer, BSize, 0); } catch(...) { if(DebugLevel(1)) DebugMessage(L"Error in Plugin: \"" + wdx_plugin[wdx_field[n].PluginIndex].Name + L"\" calling \"ContentGetValueW\""); return -7; }
        ...
        try { Result = wdx_plugin[wdx_field[n].PluginIndex].F_ContentGetValue(cBuffer2, wdx_field[n].FieldIndex, Unitindex, cBuffer, BSize, 0); } catch(...) { if(DebugLevel(1)) DebugMessage(L"Error in Plugin: \"" + wdx_plugin[wdx_field[n].PluginIndex].Name + L"\" calling \"ContentGetValue\""); return -7; }
    ...
    for(int i = 0; i < (int)wdx_plugin.size(); i++) {
        if(wdx_plugin[i].Lib) {
            if(wdx_plugin[i].F_ContentPluginUnloading) {
                ...
                try { wdx_plugin[i].F_ContentPluginUnloading(); } catch(...) { if(DebugLevel(1)) DebugMessage(L"Error in Plugin: \"" + wdx_plugin[i].Name + L"\" calling \"ContentPluginUnloading\""); }
            }
